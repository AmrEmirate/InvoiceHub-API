...
describe("Auth Flow (Register, Set Password, Login)", () => {
    ...
    describe("1. POST /api/auth/register", () => {
        it("should register a new user and ask for email verification", async () => {
            ...
            expect(res.body.message).toBe("Registration successful. Please check your email to set your password.");
    ...
    describe("2. POST /api/auth/set-password", () => {
        it("should fail if token is invalid", async () => {
            const res = await (0, supertest_1.default)(app).post("/api/auth/set-password").send({
        ...
        it("should set password and verify user successfully", async () => {
            ...
            expect(res.body.message).toBe("Password set successfully. You can now login.");
            const user = await prisma.user.findUnique({
                where: { email: testEmail },
            });
