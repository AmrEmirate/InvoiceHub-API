...
class AuthService {
    async register(input) {
        ...
        try {
            const setPasswordUrl = `${process.env.FE_URL || "http://localhost:3000"}/auth/set-password?token=${verificationToken}`;
    ...
    async forgotPassword(email) {
        ...
        try {
            const resetPasswordUrl = `${process.env.FE_URL || "http://localhost:3000"}/reset-password?token=${resetToken}`;
            await nodemailer_1.transport.sendMail({
                from: `"InvoiceHub" <${process.env.SMTP_USER}>`,
                to: user.email,
