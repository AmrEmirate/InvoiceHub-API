...
class AuthService {
  async register(input) {
    ...
    try {
      ...
      await nodemailer_1.transport.sendMail({
        ...
        html: (0, email_templates_1.generateSetPasswordEmail)(
          createdUser.name,
          setPasswordUrl
        ),
      });
  ...
  async login(input) {
    ...
    if (!user) {
      logger_1.default.warn(
        `Login attempt failed: Email ${input.email} not found.`
      );
      throw new AppError_1.default(401, "Invalid email or password");
    }
    if (!user.password) {
      logger_1.default.warn(
        `Login attempt failed: Password not set for ${input.email}.`
      );
      throw new AppError_1.default(
        403,
    ...
    if (!user.isVerified) {
      logger_1.default.warn(
        `Login attempt failed: Email ${input.email} not verified.`
      );
      throw new AppError_1.default(
        403,
    ...
    const isPasswordValid = await (0, hash_1.comparePassword)(
      input.password_plain,
      user.password
    );
    if (!isPasswordValid) {
      logger_1.default.warn(
        `Login attempt failed: Invalid password for ${input.email}.`
      );
      throw new AppError_1.default(401, "Invalid email or password");
    }
  ...
  async updateProfile(userId, data) {
    const updatedUser = await user_repository_1.default.updateUser(
      userId,
      data
    );
    logger_1.default.info(
  ...
  async handleGoogleLogin(user) {
    ...
    const token = (0, jwt_1.createToken)(tokenPayload);
    logger_1.default.info(
      `User logged in via Google: ${user.email} (ID: ${user.id})`
    );
    const { password, ...userWithoutPassword } = user;
    return { user: userWithoutPassword, token };
  }
  async googleSignup(input) {
    ...
    try {
      ...
      logger_1.default.error(
        `Database error during Google signup: ${dbError.message}`
      );
      throw new AppError_1.default(500, "Failed to create user", dbError);
    }
  ...
  async forgotPassword(email) {
    ...
    await user_repository_1.default.setResetToken(
      user.id,
      resetToken,
      resetTokenExpiry
    );
    try {
      ...
      await nodemailer_1.transport.sendMail({
        ...
        html: (0, email_templates_1.generateResetPasswordEmail)(
          user.name,
          resetPasswordUrl
        ),
      });
  ...
  async resetPassword(token, newPassword) {
    ...
    await user_repository_1.default.updatePasswordFromReset(
      user.id,
      hashedPassword
    );
    logger_1.default.info(
