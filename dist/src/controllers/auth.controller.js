...
class AuthController {
    ...
    async setPassword(req, res, next) {
        try {
            ...
            const { message } = await auth_service_1.default.setPassword(token, password);
            res.status(200).json({
                message: message,
            });
        }
    ...
    async updateMe(req, res, next) {
        try {
            ...
            const updatedUser = await auth_service_1.default.updateProfile(userId, req.body);
            res.status(200).json({
    ...
    async googleCallback(req, res, next) {
        try {
            ...
            if (userData.isNewUser) {
                ...
                res.redirect(`${feUrl}/auth/callback?newUser=true&googleEmail=${encodedEmail}&googleName=${encodedName}&googleId=${encodedGoogleId}`);
                return;
            }
            const { user, token } = await auth_service_1.default.handleGoogleLogin(userData);
            const userDataEncoded = encodeURIComponent(JSON.stringify(user));
            res.redirect(`${feUrl}/auth/callback?token=${token}&user=${userDataEncoded}`);
        }
        catch (error) {
            next(error);
        }
